...
class rerunMVAIsolationOnMiniAOD : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   public:
      ...
      reco::CandidatePtrVector getGammas_pat(const pat::Tau& tau, bool signal = true);
      bool isInside_pat(float photon_pt, float deta, float dphi);
      float tau_Eratio_pat(const pat::Tau& tau);
      float pt_weighted_dx_pat(const pat::Tau& tau, int mode = 0, int var = 0, int decaymode = -1);
      float tau_pt_weighted_dr_signal_pat(const pat::Tau& tau, int dm);
      float tau_pt_weighted_deta_strip_pat(const pat::Tau& tau, int dm);
      float tau_pt_weighted_dphi_strip_pat(const pat::Tau& tau, int dm);
      float tau_pt_weighted_dr_iso_pat(const pat::Tau& tau, int dm);
      unsigned int tau_n_photons_total_pat(const pat::Tau& tau);
...

{
   ...
   pfTauToken_ = consumes<reco::PFTauCollection>(edm::InputTag("hpsPFTauProducer76xReMiniAOD","","PAT"));
   dmfNewToken_ = consumes<reco::PFTauDiscriminator>(edm::InputTag("hpsPFTauDiscriminationByDecayModeFindingNewDMs76xReMiniAOD","","PAT"));

   chargedIsoPtSumToken_ = consumes<reco::PFTauDiscriminator>(edm::InputTag("hpsPFTauChargedIsoPtSum76xReMiniAOD","","PAT"));
   neutralIsoPtSumToken_ = consumes<reco::PFTauDiscriminator>(edm::InputTag("hpsPFTauNeutralIsoPtSum76xReMiniAOD","","PAT"));
   puCorrPtSumToken_ = consumes<reco::PFTauDiscriminator>(edm::InputTag("hpsPFTauPUcorrPtSum76xReMiniAOD","","PAT"));
   photonPtSumOutsideSignalConeToken_ = consumes<reco::PFTauDiscriminator>(edm::InputTag("hpsPFTauPhotonPtSumOutsideSignalCone76xReMiniAOD","","PAT"));
   footprintCorrectionToken_ = consumes<reco::PFTauDiscriminator>(edm::InputTag("hpsPFTauFootprintCorrection76xReMiniAOD","","PAT"));
   tauTIPToken_ = consumes<PFTauTIPAssociationByRef>(edm::InputTag("hpsPFTauTransverseImpactParameters76xReMiniAOD","","PAT"));
	...
	if(additionalCollectionsAvailable_)
	{
		...
		for(unsigned iPFTau = 0; iPFTau < pfTaus->size(); iPFTau++)
		{
			...
			for(unsigned iTau = 0; iTau < unmatchedTaus.size(); iTau++)
			{
				...
				nPhoton->Fill((float)tau_n_photons_total(*pfTau),(float)tau_n_photons_total_pat(*unmatchedTaus.at(iTau)));
				ptWeightedDetaStrip->Fill(tau_pt_weighted_deta_strip(*pfTau, pfTau->decayMode()),tau_pt_weighted_deta_strip_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode()));
				ptWeightedDphiStrip->Fill(tau_pt_weighted_dphi_strip(*pfTau, pfTau->decayMode()),tau_pt_weighted_dphi_strip_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode()));
				ptWeightedDrSignal->Fill(tau_pt_weighted_dr_signal(*pfTau, pfTau->decayMode()),tau_pt_weighted_dr_signal_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode()));
				ptWeightedDrIsolation->Fill(tau_pt_weighted_dr_iso(*pfTau, pfTau->decayMode()),tau_pt_weighted_dr_iso_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode()));
				leadTrackChi2->Fill(tau_leadTrackChi2(*pfTau),unmatchedTaus.at(iTau)->leadingTrackNormChi2());
				eRatio->Fill(tau_Eratio(*pfTau),tau_Eratio_pat(*unmatchedTaus.at(iTau)));
				...
				if((float)tau_n_photons_total(*pfTau) != (float)tau_n_photons_total_pat(*unmatchedTaus.at(iTau))){
					if(verbosity_) std::cout << "nPhoton PF: = " << (float)tau_n_photons_total(*pfTau) << ", pat = " << (float)tau_n_photons_total_pat(*unmatchedTaus.at(iTau)) << std::endl;
					differences->Fill(17);
					differencesWeighted->Fill(17,fabs((float)tau_n_photons_total(*pfTau)-(float)tau_n_photons_total_pat(*unmatchedTaus.at(iTau))));
				}
				if(tau_pt_weighted_deta_strip(*pfTau, pfTau->decayMode()) != tau_pt_weighted_deta_strip_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode())){
					if(verbosity_) std::cout << "ptWeightedDetaStrip: PF = " << tau_pt_weighted_deta_strip(*pfTau, pfTau->decayMode()) << ", pat = " << tau_pt_weighted_deta_strip_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode()) << std::endl;
					differences->Fill(18);
					differencesWeighted->Fill(18,fabs(tau_pt_weighted_deta_strip(*pfTau, pfTau->decayMode())-tau_pt_weighted_deta_strip_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode())));
					difference_ptWeightedDetaStrip->Fill(fabs(tau_pt_weighted_deta_strip(*pfTau, pfTau->decayMode())-tau_pt_weighted_deta_strip_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode())));
				}
				if(tau_pt_weighted_dphi_strip(*pfTau, pfTau->decayMode()) != tau_pt_weighted_dphi_strip_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode())){
					if(verbosity_) std::cout << "ptWeightedDphiStrip: PF = " << tau_pt_weighted_dphi_strip(*pfTau, pfTau->decayMode()) << ", pat = " << tau_pt_weighted_dphi_strip_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode()) << std::endl;
					differences->Fill(19);
					differencesWeighted->Fill(19,fabs(tau_pt_weighted_dphi_strip(*pfTau, pfTau->decayMode())-tau_pt_weighted_dphi_strip_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode())));
					difference_ptWeightedDphiStrip->Fill(fabs(tau_pt_weighted_dphi_strip(*pfTau, pfTau->decayMode())-tau_pt_weighted_dphi_strip_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode())));
				}
				if(tau_pt_weighted_dr_signal(*pfTau, pfTau->decayMode()) != tau_pt_weighted_dr_signal_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode())){
					if(verbosity_) std::cout << "ptWeightedDrSignal: PF = " << tau_pt_weighted_dr_signal(*pfTau, pfTau->decayMode()) << ", pat = " << tau_pt_weighted_dr_signal_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode()) << std::endl;
					differences->Fill(20);
					differencesWeighted->Fill(20,fabs(tau_pt_weighted_dr_signal(*pfTau, pfTau->decayMode())-tau_pt_weighted_dr_signal_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode())));
					difference_ptWeightedDrSignal->Fill(fabs(tau_pt_weighted_dr_signal(*pfTau, pfTau->decayMode())-tau_pt_weighted_dr_signal_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode())));
				}
				if(tau_pt_weighted_dr_iso(*pfTau, pfTau->decayMode()) != tau_pt_weighted_dr_iso_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode())){
					if(verbosity_) std::cout << "ptWeightedDrIso: PF = " << tau_pt_weighted_dr_iso(*pfTau, pfTau->decayMode()) << ", pat = " << tau_pt_weighted_dr_iso_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode()) << std::endl;
					differences->Fill(21);
					differencesWeighted->Fill(21,fabs(tau_pt_weighted_dr_iso(*pfTau, pfTau->decayMode())-tau_pt_weighted_dr_iso_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode())));
					difference_ptWeightedDrIso->Fill(fabs(tau_pt_weighted_dr_iso(*pfTau, pfTau->decayMode())-tau_pt_weighted_dr_iso_pat(*unmatchedTaus.at(iTau),unmatchedTaus.at(iTau)->decayMode())));
				}
				if(tau_leadTrackChi2(*pfTau) != unmatchedTaus.at(iTau)->leadingTrackNormChi2()){
					if(verbosity_) std::cout << "leadTrackChi2: PF = " << tau_leadTrackChi2(*pfTau) << ", pat = " << unmatchedTaus.at(iTau)->leadingTrackNormChi2() << std::endl;
					differences->Fill(22);
					differencesWeighted->Fill(22,fabs(tau_leadTrackChi2(*pfTau)-unmatchedTaus.at(iTau)->leadingTrackNormChi2()));
				}
				if(tau_Eratio(*pfTau) != tau_Eratio_pat(*unmatchedTaus.at(iTau))){
					if(verbosity_) std::cout << "eRatio: PF = " << tau_Eratio(*pfTau) << ", pat = " << tau_Eratio_pat(*unmatchedTaus.at(iTau)) << std::endl;
					differences->Fill(23);
					differencesWeighted->Fill(23,fabs(tau_Eratio(*pfTau)-tau_Eratio_pat(*unmatchedTaus.at(iTau))));
				}
				if(verbosity_) std::cout << "=============================================================" << std::endl;
			}
		}
...
// these are copies from RecoTauTag/RecoTau/interface/PATTauClusterVariables.h
// because this class can't be included simultaneously with RecoTauTag/RecoTau/interface/PFTauClusterVariables.h

reco::CandidatePtrVector rerunMVAIsolationOnMiniAOD::getGammas_pat(const pat::Tau& tau, bool signal){
	if(signal)
		return tau.signalGammaCands();
	return tau.isolationGammaCands();
}

bool rerunMVAIsolationOnMiniAOD::isInside_pat(float photon_pt, float deta, float dphi){
...
float rerunMVAIsolationOnMiniAOD::tau_Eratio_pat(const pat::Tau& tau){
	...
	if(total == 0) return -1;
	else return EcalEnInSignalCands/total;
}

float rerunMVAIsolationOnMiniAOD::pt_weighted_dx_pat(const pat::Tau& tau, int mode, int var, int decaymode){
    float sum_pt = 0.;
    float sum_dx_pt = 0.;
    float signalrad = std::max(0.05, std::min(0.1, 3./std::max(1., tau.pt())));
    int is3prong = (decaymode==10);

    auto cands = getGammas_pat(tau, mode < 2);
...
float rerunMVAIsolationOnMiniAOD::tau_pt_weighted_dr_signal_pat(const pat::Tau& tau, int dm){
	return pt_weighted_dx_pat(tau, 0, 0, dm);
}

float rerunMVAIsolationOnMiniAOD::tau_pt_weighted_deta_strip_pat(const pat::Tau& tau, int dm){
	if(dm == 10)
		return pt_weighted_dx_pat(tau, 2, 1, dm);
	return pt_weighted_dx_pat(tau, 1, 1, dm);
}

float rerunMVAIsolationOnMiniAOD::tau_pt_weighted_dphi_strip_pat(const pat::Tau& tau, int dm){
	if(dm == 10)
		return pt_weighted_dx_pat(tau, 2, 2, dm);
	return pt_weighted_dx_pat(tau, 1, 2, dm);
}

float rerunMVAIsolationOnMiniAOD::tau_pt_weighted_dr_iso_pat(const pat::Tau& tau, int dm){
	return pt_weighted_dx_pat(tau, 2, 0, dm);
}

unsigned int rerunMVAIsolationOnMiniAOD::tau_n_photons_total_pat(const pat::Tau& tau){
