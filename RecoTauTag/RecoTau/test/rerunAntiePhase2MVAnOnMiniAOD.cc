// -*- C++ -*-
//
// Package:    RecoTauTag/RecoTau/rerunAntiePhase2MVAnOnMiniAOD
// Class:      rerunAntiePhase2MVAnOnMiniAOD
//
/**\class rerunAntiePhase2MVAnOnMiniAOD rerunAntiePhase2MVAnOnMiniAOD.cc RecoTauTag/RecoTau/test/rerunAntiePhase2MVAnOnMiniAOD.cc
...
class rerunAntiePhase2MVAnOnMiniAOD : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
  explicit rerunAntiePhase2MVAnOnMiniAOD(const edm::ParameterSet&);
  ~rerunAntiePhase2MVAnOnMiniAOD();
...
private:
  ...
  TH1D* mvaValueAOD; 
  TH1D* mvaValueAODClean;
  TH1D* mvaValueMiniAOD;
  TH1D* mvaValueMiniAODClean;
  TH1D* mvaValueDiff;
...
//
rerunAntiePhase2MVAnOnMiniAOD::rerunAntiePhase2MVAnOnMiniAOD(const edm::ParameterSet& iConfig)

{
  ...
  mvaValueAOD = fileService->make<TH1D>("mvaValueAOD", ";MVA value;", 101, -100, 1.1);
  mvaValueAODClean = fileService->make<TH1D>("mvaValueAODClean", ";MVA value;", 120, -1.1, 1.1);
  mvaValueMiniAOD = fileService->make<TH1D>("mvaValueMiniAOD", ";MVA value;", 101, -100., 1.1);
  mvaValueMiniAODClean = fileService->make<TH1D>("mvaValueMiniAODClean", ";MVA value;", 120, -1.1, 1.1);
  mvaValueDiff = fileService->make<TH1D>("mvaValueDiff", ";|Reco - MiniAOD|;", 2200, -110, 110);
  ...
  mvaValue_antiEMVA6 = fileService->make<TH2D>("mvaValue_antiE", ";reco;MiniAOD", 220, -1.1, 1.1, 220, -1.1, 1.1);
}

rerunAntiePhase2MVAnOnMiniAOD::~rerunAntiePhase2MVAnOnMiniAOD() {
...
void rerunAntiePhase2MVAnOnMiniAOD::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (unsigned iTau = 0; iTau < taus->size(); iTau++) {
    ...
    float valueAOD =  tau->tauID("againstElectronMVA6Raw");
    float valueMiniAOD = tau->tauID("againstElectronPhase2MVA6v1Raw");
    
    mvaValueAOD->Fill(valueAOD);
    if (valueAOD> -99.) mvaValueAODClean->Fill(valueAOD);
    
    mvaValueMiniAOD->Fill(valueMiniAOD);
    if (valueMiniAOD> -99.) mvaValueMiniAODClean->Fill(valueMiniAOD);
   ...
   if (valueMiniAOD> -99. && valueAOD> -99.)  
   mvaValue_antiEMVA6->Fill(valueMiniAOD, valueAOD);
...
void rerunAntiePhase2MVAnOnMiniAOD::beginJob() {}

// ------------ method called once each job just after ending the event loop  ------------
void rerunAntiePhase2MVAnOnMiniAOD::endJob() {}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void rerunAntiePhase2MVAnOnMiniAOD::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(rerunAntiePhase2MVAnOnMiniAOD);
