...
double PFRecoTauDiscriminationByMVAIsolationRun2::discriminate(const PFTauRef& tau) const
{
  ...
  if ( ((mvaOpt_ == kOldDMwoLT || mvaOpt_ == kOldDMwLT || mvaOpt_ == kDBoldDMwLT || mvaOpt_ == kPWoldDMwLT) && (tauDecayMode == 0 || tauDecayMode == 1 || tauDecayMode == 2 || tauDecayMode == 10)) ||
    ...
    float dxy = tauLifetimeInfo.dxy();
    float dxyErr = tauLifetimeInfo.dxy_error();
    float dxySig = 0.;
    if(dxyErr != 0)
    	dxySig = dxy / dxyErr;
    float ip3d = tauLifetimeInfo.ip3d();
    float ip3dErr = tauLifetimeInfo.ip3d_error();
    float ip3dSig = 0.;
    if(ip3dErr != 0)
    ...
    float leadingTrackChi2 = tau_leadTrackChi2(*tau);
    ...
    if ( mvaOpt_ == kOldDMwoLT || mvaOpt_ == kNewDMwoLT ) {
      ...
      mvaInput_[4]  = std::log(std::max((float)1.e-2, puCorrPtSum));
      mvaInput_[5]  = tauDecayMode;
      mvaInput_[6]  = TMath::Sign((float)+1., dxy);
      mvaInput_[7]  = std::sqrt(std::fabs(std::min((float)1., dxy)));
      mvaInput_[8]  = std::min((float)10., std::fabs(dxySig));
      mvaInput_[9]  = ( tauLifetimeInfo.hasSecondaryVertex() ) ? 1. : 0.;
      mvaInput_[10] = std::sqrt(decayDistMag);
      mvaInput_[11] = std::min((float)10., (float)tauLifetimeInfo.flightLengthSig());
    } else if ( mvaOpt_ == kDBoldDMwLT || mvaOpt_ == kDBnewDMwLT ) {
      mvaInput_[0]  = std::log(std::max((float)1., (float)tau->pt()));
      mvaInput_[1]  = std::fabs((float)tau->eta());
      mvaInput_[2]  = std::log(std::max((float)1.e-2, chargedIsoPtSum));
      ...
      mvaInput_[14]  = TMath::Sign((float)+1., dxy);
      mvaInput_[15]  = std::sqrt(std::fabs(std::min((float)1., std::fabs(dxy))));
      mvaInput_[16]  = std::min((float)10., std::fabs(dxySig));
      mvaInput_[17]  = TMath::Sign((float)+1., ip3d);
      mvaInput_[18]  = std::sqrt(std::fabs(std::min((float)1., std::fabs(ip3d))));
      mvaInput_[19]  = std::min((float)10., std::fabs(ip3dSig));
