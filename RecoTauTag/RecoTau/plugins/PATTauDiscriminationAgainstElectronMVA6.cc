...
double PATTauDiscriminationAgainstElectronMVA6::discriminate(const TauRef& theTauRef) const
{
  ...
  if( (*theTauRef).leadChargedHadrCand().isNonnull()) {
    ...
    for ( reco::CandidatePtrVector::const_iterator gamma = signalGammaCands.begin();
	  gamma != signalGammaCands.end(); ++gamma ) {
  ...
  if ( verbosity_ ) {
    edm::LogPrint("PFTauAgainstEleMVA6") << "<PATTauDiscriminationAgainstElectronMVA6::discriminate>:" ;
    edm::LogPrint("PFTauAgainstEleMVA6") << " tau: Pt = " << theTauRef->pt() << ", eta = " << theTauRef->eta() << ", phi = " << theTauRef->phi();
    edm::LogPrint("PFTauAgainstEleMVA6") << " deltaREleTau = " << deltaRDummy << ", isGsfElectronMatched = " << isGsfElectronMatched;
    edm::LogPrint("PFTauAgainstEleMVA6") << " #Prongs = " << theTauRef->signalChargedHadrCands().size();
